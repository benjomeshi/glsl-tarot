...
void main(){
  ...
  uv += rotate(vec2(.3, -.15), angle);
  uv *= 1.5;
  uv = rotate(uv, angle);
  uv.y = flip(uv.y, step(.0, uv.x));
  float tri_1 = polySDF(uv, 3);
  uv.y = 1. - uv.y;
  float tri_2 = polySDF(uv, 3);

  color += stroke(tri_1, .4, .15);
  color = bridge(color, tri_2, .4, .15);

...
void main(){
  ...
  vec2 scale = vec2(1.);
  vec2 offset = vec2(.1015, .0);

  uv.x = mix(-uv.x, uv.x, step(.0, uv.y));

  float angle = PI/4.;

  float left = rectSDF(rotate(uv+offset, angle), scale);
  float right = rectSDF(rotate(uv-offset, angle), scale);

  color += stroke(left, .145, .098);
  color = bridge(color, right, .145, .098);
